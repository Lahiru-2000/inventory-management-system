...
const LowStockReport = () => {
  ...
  const downloadPDF = async () => {
    setLoading(true)
    try {
      const response = await api.get('/reports/export/low-stock/pdf', {
        responseType: 'blob'
      })
  ...
  const downloadExcel = async () => {
    setLoading(true)
    try {
      const response = await api.get('/reports/export/low-stock/excel', {
        responseType: 'blob'
      })
      ...
      const link = document.createElement('a')
      link.href = window.URL.createObjectURL(blob)
      link.download = 'low-stock-report.xlsx'
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Low Stock Items Report</h1>
